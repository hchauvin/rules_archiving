...
def execute_commands(config, platform, git_repository, use_but, save_but,
    ...
    try:
        ...
        execute_shell_commands(
          config.get("shell_commands", None), 
          group = ":bash: Setup (Shell Commands)")
        ...
        if (not fail_pipeline) and (not build_only):
            ...
            try:
                ...
                if has_flaky_tests(test_bep_file):
                    ...
                    # raise BazelTestFailedException ==> should be parameter
                if is_pull_request():
        ...
        if (not fail_pipeline) and (not build_only):
            execute_shell_commands(
              config.get("e2e_bazel", None),
              group=":bazel: Bazel end-to-end tests")
    finally:
        if tmpdir:
...
def execute_shell_commands(commands, group):
    if not commands:
        return
    print_collapsed_group(group)
    shell_command = "\n".join(commands)
...
def execute_bazel_run(bazel_binary, targets):
    ...
    for target in targets:
        execute_command([bazel_binary, "--host_jvm_args=-Dbazel.DigestFunction=SHA256", "run", "--curses=yes",
...
def remote_caching_flags(platform):
    ...
    return [
      "--remote_timeout=10", 
      "--experimental_remote_spawn_cache", 
      "--remote_rest_cache=http://localhost:8080/cache",
    ]
...
def execute_bazel_build(bazel_binary, platform, flags, targets, bep_file):
    ...
    try:
        execute_command([bazel_binary, "--host_jvm_args=-Dbazel.DigestFunction=SHA256", "build"] +
...
def execute_bazel_test(bazel_binary, platform, flags, targets, bep_file):
    if not targets:
        # No test target to execute, as odd as it seems
        return
    ...
    if not remote_enabled(flags):
        caching_flags = remote_caching_flags(platform)
    try:
        execute_command([bazel_binary, "--host_jvm_args=-Dbazel.DigestFunction=SHA256", "test"] +
                        common_flags + caching_flags + flags + targets)
    except subprocess.CalledProcessError as e:
